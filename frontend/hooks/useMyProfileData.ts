...
const POST_SELECT_V1 =
  "id, caption, image_url, created_at, hashtags, category, visibility, status, is_edited, comments_enabled, comments_count, is_pinned, thumbnail_url, allow_downloads, allow_remix, allow_duet";
...
export function useMyPosts(userId: string | null) {
  return useQuery({
    ...
    queryFn: async (): Promise<ProfilePost[]> => {
      if (!userId) return [];
      for (const select of [POST_SELECT_FULL, POST_SELECT_V1, POST_SELECT_LEGACY]) {
        const { data, error } = await supabase
          .from("posts")
          .select(select)
          .eq("author_id", userId)
          .order("created_at", { ascending: false });
        if (!error) return ((data ?? []) as Record<string, unknown>[]).map(normalizePost);
      }
